  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <h1 className="mt-2 text-[20px] font-bold text-ink">Channels</h1>
        <p className="mt-0.5 text-[13px] text-ink/45">Connect and manage your messaging platforms.</p>
      </div>
        ...
        {channels.map((c) => (
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-[12px] text-ink/40">{c.conversations} conversations</p>
            </div>
            <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${
              c.status === "connected" ? "bg-emerald-50 text-emerald-600" : "bg-ink/[0.04] text-ink/40"
            }`}>{c.status}</span>
          </Link>
        ))}
      </div>
