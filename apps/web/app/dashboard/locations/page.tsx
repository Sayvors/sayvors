...
const MOCK_LOCATIONS: LocationOption[] = [
  ...
  { id: "loc_2", name: "Sayvors Olaya", address: "Olaya Street, Riyadh", status: "active" },
];
...
function LocationsInner() {
  ...
  const urlBanner = useMemo(() => {
    ...
    if (created) return { kind: "ok" as const, text: "Location created successfully." };
    return null;
  }, [params]);
  ...
  const dismissBanner = useCallback(() => {
    setBanner(null);
    try { window.history.replaceState(null, "", "/dashboard/locations"); } catch {}
  }, []);
  ...
  const handleDelete = async () => {
    if (!selectedId || !confirm("Delete this location? This cannot be undone.")) return;
    setDeleting(true);
    try {
      await apiFetch(`/api/v1/locations/${selectedId}`, { method: "DELETE" });
      setLocations((prev) => prev.filter((l) => l.id !== selectedId));
      setSelectedId(locations.find((l) => l.id !== selectedId)?.id ?? null);
      setBanner({ kind: "ok", text: "Location deleted." });
    } catch {
      setBanner({ kind: "err", text: "Could not delete location." });
    } finally {
      setDeleting(false);
    }
  };
  ...
  return (
    <div className="h-full overflow-y-auto p-6 space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="rounded-xl bg-deep-violet px-4 py-2 text-[13px] font-semibold text-white transition hover:opacity-90"
          >
            + Add Location
          </button>
        </div>
      ...
      {/* Banner */}
      {activeBanner && (
        <div className={`rounded-xl border p-3 text-[13px] ${activeBanner.kind === "ok" ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-red-200 bg-red-50 text-red-700"}`}>
          <div className="flex items-center justify-between gap-3">
            <span>{activeBanner.text}</span>
            <button className="shrink-0 text-[12px] underline underline-offset-2" onClick={dismissBanner}>dismiss</button>
          </div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-ink/[0.12] bg-white/[0.02] py-20 dark:border-fog/[0.12]">
          <LocationIcon className="mb-3 h-10 w-10 text-ink/20 dark:text-fog/20" />
          <p className="text-[14px] font-medium text-ink/50 dark:text-fog/50">No locations yet</p>
          <p className="mt-1 text-[12px] text-ink/35 dark:text-fog/35">Add your first business location to get started.</p>
          <button
            onClick={() => setShowAddModal(true)}
            className="mt-4 rounded-xl bg-deep-violet px-4 py-2 text-[13px] font-semibold text-white transition hover:opacity-90"
          >
            + Add Location
          </button>
        </div>
        ...
        <>
          ...
          {/* Delete zone */}
          <div className="flex items-center justify-between rounded-2xl border border-red-200/60 bg-red-50/40 p-4 dark:border-red-500/20 dark:bg-red-500/[0.04]">
            <div>
              <p className="text-[13px] font-medium text-red-700 dark:text-red-400">Danger zone</p>
              <p className="text-[11px] text-red-500/60 dark:text-red-400/50">Permanently delete this location. This action cannot be undone.</p>
            </div>
            <button
              onClick={handleDelete}
              disabled={deleting}
              className="rounded-lg border border-red-300 bg-white px-3 py-1.5 text-[12px] font-semibold text-red-600 transition hover:bg-red-50 dark:border-red-500/30 dark:bg-transparent dark:text-red-400 dark:hover:bg-red-500/10 disabled:opacity-50"
            >
              {deleting ? <span className="inline-flex items-center gap-1"><LogoLoader size={12} /> Deleting...</span> : "Delete Location"}
            </button>
          </div>
        </>
      )}
...
function DetailsTab({ location, onSave }: { location: LocationOption | null; onSave: () => void }) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await apiFetch(`/api/v1/locations/${location?.id}`, {
        method: "PUT",
        body: JSON.stringify({ name, address, phone, website }),
      });
      onSave();
...
function CategoriesTab({ location, onSave }: { location: LocationOption | null; onSave: () => void }) {
  const [primary, setPrimary] = useState(location?.name ?? "");
...
function ServiceAreaTab({ location, onSave }: { location: LocationOption | null; onSave: () => void }) {
  const [areas, setAreas] = useState<string[]>(["Al Malqa", "Riyadh"]);
...
function DescriptionTab({ location, onSave }: { location: LocationOption | null; onSave: () => void }) {
  const [desc, setDesc] = useState("We are a premium AI-powered customer service platform helping businesses manage their online presence.");
...
function AddLocationModal({ onClose }: { onClose: () => void }) {
  ...
  const handleCreate = async () => {
    ...
    try {
      await apiFetch("/api/v1/locations/", {
        method: "POST",
        body: JSON.stringify({ name, address, phone, website, primary_category: category }),
      });
      onClose();
      window.location.href = "/dashboard/locations?created=1";
    } catch {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <div
        className="w-full max-w-lg rounded-2xl border border-ink/[0.08] bg-white p-6 shadow-2xl dark:border-fog/[0.1] dark:bg-ink"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="mb-5 flex items-center justify-between">
          <h2 className="text-[16px] font-bold text-ink dark:text-fog">Add Location</h2>
          <button onClick={onClose} className="text-ink/30 transition hover:text-ink/60 dark:text-fog/30">
        ...
        <div className="space-y-4">
          <Field label="Business Name *">
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. My Business" className="input-field" autoFocus />
          </Field>
          <Field label="Address *">
            <input value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Full address" className="input-field" />
          </Field>
          ...
          <Field label="Primary Category">
            <input value={category} onChange={(e) => setCategory(e.target.value)} placeholder="e.g. Restaurant, Plumber" className="input-field" />
          </Field>
        </div>
        <div className="mt-6 flex justify-end gap-2">
          <button onClick={onClose} className="btn-secondary">Cancel</button>
          <button onClick={handleCreate} disabled={!name.trim() || !address.trim() || saving} className="btn-primary disabled:opacity-50">
            {saving ? <span className="inline-flex items-center gap-1.5"><LogoLoader size={14} /> Creating...</span> : "Create Location"}
          </button>
        </div>
      </div>
    </div>
