...
export default function SpecificConversationPage({ params }: { params: Promise<{ platform: string; id: string }> }) {
  const { platform: rawPlatform, id } = use(params);
    ...
    <div className="flex h-[calc(100vh-8rem)] flex-col">
      <div className="mb-4">
        <Breadcrumbs
          items={[
            ...
            { label: platform, href: `/dashboard/conversations/${rawPlatform}` },
            { label: `Chat #${id}` },
          ]}
        />
        <h1 className="mt-2 text-[18px] font-bold text-ink dark:text-fog">Sarah Chen</h1>
        <p className="text-[12px] text-ink/40 dark:text-fog/40">{platform} · Active now</p>
      </div>

      <div className="flex-1 overflow-y-auto rounded-xl border border-ink/[0.06] bg-white p-4 dark:border-fog/[0.06] dark:bg-ink">
        <div className="space-y-4">
          {dummyMessages.map((m) => (
            <div key={m.id} className={`flex ${m.role === "user" ? "justify-start" : "justify-end"}`}>
              <div className={`max-w-[70%] rounded-2xl px-4 py-2.5 ${
                m.role === "user"
                  ? "bg-ink/[0.04] text-ink dark:bg-fog/[0.04] dark:text-fog"
                  : "bg-gradient-to-r from-deep-violet to-magenta text-white"
              }`}>
                <p className="text-[13px]">{m.content}</p>
                <p className={`mt-1 text-[10px] ${m.role === "user" ? "text-ink/30 dark:text-fog/30" : "text-white/50"}`}>{m.time}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="mt-3 flex gap-2">
        <input
          ...
          className="flex-1 rounded-xl border border-ink/[0.08] bg-white px-4 py-2.5 text-[13px] text-ink outline-none transition placeholder:text-ink/30 focus:border-deep-violet/30 focus:ring-2 focus:ring-deep-violet/[0.06] dark:border-fog/[0.08] dark:bg-ink dark:text-fog dark:placeholder:text-fog/30"
        />
